...
async function finishGame(kind){
  ...
  state.gamesPlayed++;save();if(currentUser){await sb.from("user_game_history").insert({user_id:currentUser.id,game_type:kind,score:gameStats.right,total:gameStats.right+gameStats.wrong});await syncCloudProgress()}
